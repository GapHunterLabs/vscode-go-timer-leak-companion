 ...
 * a real number of distinct timer leaks flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'goTimerLeakCompanion.review.seenFindings';
const KEY_ANSWERED = 'goTimerLeakCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Go Timer Leak Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
